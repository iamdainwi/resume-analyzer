...
export default function Home() {
    ...
    const check = async () => {
      try {
        const res = await axios.get(
          `https://resume-analyzer-nine-lyart.vercel.app/`,
          { timeout: 3000 },
        );
  ...
  const startJob = async () => {
    if (!files || !jd.trim()) {
      setError(
        "Please provide a job description and upload at least one resume.",
      );
      return;
    }
    setLoading(true);
    ...
    try {
      const res = await axios.post(
        `https://resume-analyzer-nine-lyart.vercel.app/start-job`,
        formData,
        {
          ...
          <CardContent>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              {jd.length > 0
                ...
                : "Paste or type the full job requirements"}
            </p>
          </CardContent>
        </Card>
          ...
          <Alert className="animate-fade-up border-warning/30 bg-warning-muted text-warning">
            ...
            <div>
              ...
              <AlertDescription className="text-warning/80">
                Resume analysis will use basic keyword matching. Start the
                backend for AI-powered results.
              </AlertDescription>
            </div>
          </Alert>
        )}
                  ...
                  <CardContent className="pt-6">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="shrink-0 text-right">
                        <span
                          className={`text-2xl font-bold tabular-nums ${scoreColor(r.score)}`}
                        >
                          {r.score}
                        </span>
                        <span className={`text-sm ${scoreColor(r.score)}`}>
                          /100
                        </span>
                      </div>
                    </div>
                          ...
                          {r.jd_keywords && r.jd_keywords.length > 0 && (
                            <div className="flex flex-wrap gap-1.5">
                              {r.jd_keywords.slice(0, 6).map((kw, idx) => {
                                const matched =
                                  r.matched_keywords?.includes(kw);
                                return (
                                  <Badge
                                    key={idx}
                                    variant="outline"
